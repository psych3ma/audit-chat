...
# 조문 패턴: "제21조", "제23조의2" 등
_ARTICLE_PATTERN = re.compile(r"^(.+?)\s+(제\d+조의?\d*)\s*$")
...
def _build_article_url(law_name: str, article: str) -> str:
    ...
    segments = ["법령", law_name, article]
    path = "/".join(quote(seg, safe="") for seg in segments)
    return f"{LAW_GO_KR_ARTICLE_BASE}/{path}"
...
def _normalize_candidates(raw: str) -> list[str]:
    ...
    if not s:
        return []
    candidates = [s]
    no_jo = re.sub(r"\s*제\d+조의?\d*\s*$", "", s).strip()
...
def get_law_url(law_name: str) -> Optional[str]:
    ...
    if parsed:
        ...
        for candidate in _normalize_candidates(s):
            ...
            name_for_lookup = re.sub(r"\s*제\d+조의?\d*\s*$", "", candidate).strip() or candidate
