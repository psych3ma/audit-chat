...
**참고:** 데모/MVP 단계에서는 단일 파일이 배포 및 관리에 더 간편할 수 있음
...
**참고:** 2개 변수에 상태관리 패턴은 과잉일 수 있음

---

### 9. Mermaid Shape Map 하드코딩 *(선택사항)*
...
| 순서 | 우선순위 | 항목 | 파일 |
|------|---------|------|------|
| 1 | P1 | Magic numbers → config.py | backend/config.py |
| 2 | P1 | 외부 URL → config | config.py, law_registry.py |
| 3 | P1 | 타입 힌트 보강 | routers/*.py, services/*.py |
| 4 | P2 | CSS 변수 정리 | audit-chat-pwc.html |
| 5 | P2 | JSDoc 추가 | audit-chat-pwc.html |
| 6 | P2 *(선택)* | i18n 객체 도입 | audit-chat-pwc.html |
| 7 | P2 *(선택)* | 파일 분리 | static/ |
